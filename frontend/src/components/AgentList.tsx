...
export const AgentList = ({ limit = 8, showPremiumBadge = true }: AgentListProps) => {
  ...
  return React.createElement('div',
      ...
      displayAgents.map(agent =>
        React.createElement('div',
          ...
          React.createElement('div', { className: 'flex items-center gap-3 flex-1 min-w-0' },
            React.createElement('div', 
              { className: `p-2 rounded-lg ${getRoleColor(agent.role)}` },
              React.createElement(Zap, { size: 16 })
            ),
            React.createElement('div', { className: 'min-w-0 flex-1' },
              React.createElement('div', { className: 'flex items-center gap-2' },
                React.createElement('p', { className: 'font-medium text-white truncate' }, agent.displayName),
                showPremiumBadge && agent.isPremium && React.createElement(Crown, { 
                  size: 14, 
    ...
    agents.length > limit && React.createElement('div', 
