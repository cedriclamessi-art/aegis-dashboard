  ...
  return React.createElement('div',
    ...
    React.createElement('div', { className: 'flex items-start justify-between' },
      React.createElement('div',
        { className: 'flex-1' },
        React.createElement('p', { className: 'text-sm text-slate-400 mb-2' }, title),
        React.createElement('div', { className: 'flex items-baseline gap-2' },
          React.createElement('h3', { className: 'text-3xl font-bold text-white' }, value),
          trend !== undefined && React.createElement('span',
            { className: `flex items-center gap-1 text-sm ${trend >= 0 ? 'text-emerald-400' : 'text-red-400'}` },
            React.createElement(trend >= 0 ? TrendingUp : TrendingDown, { size: 16 }),
            `${Math.abs(trend)}%`
          )
        )
      ),
