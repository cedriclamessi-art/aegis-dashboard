...
import {
  ...
  DollarSign, Filter, Globe, LineChart, RefreshCw,
  Target, TrendingUp, Zap, Bell, Clock
} from 'lucide-react'
  ...
  return React.createElement('div', { className: 'min-h-screen bg-[var(--cyber-dark)] cyber-grid p-6' },
    React.createElement('div', { className: 'max-w-[1600px] mx-auto space-y-6' },
      ...
      React.createElement('div', { className: 'flex flex-col md:flex-row md:items-center justify-between gap-4' },
        ...
        React.createElement('div', { className: 'flex items-center gap-3' },
          React.createElement('div', { className: 'flex items-center gap-2 cyber-card px-3 py-2' },
            React.createElement(Calendar, { size: 16, className: 'text-cyan-400' }),
          ...
          React.createElement('div', { key: idx, className: 'cyber-card p-5' },
            React.createElement('div', { className: 'flex items-start justify-between mb-3' },
              ...
              React.createElement('div', {
                ...
                React.createElement(stat.change > 0 ? ArrowUpRight : ArrowDownRight, { size: 14 }),
                `${stat.change}%`
              )
            ),
            React.createElement('p', { className: 'text-xs text-slate-500 font-mono uppercase mb-1' }, stat.label),
            React.createElement('p', { className: 'text-2xl font-bold text-white' }, stat.value)
          )
        )
      ),
      ...
      React.createElement('div', { className: 'grid grid-cols-1 lg:grid-cols-3 gap-6' },
        ...
        React.createElement('div', { className: 'lg:col-span-2 cyber-card p-6' },
          ...
          React.createElement('div', { className: 'h-64 flex items-end justify-between gap-2' },
            PERFORMANCE_DATA.map((day, idx) =>
              React.createElement('div', { key: idx, className: 'flex-1 flex flex-col items-center gap-2' },
                React.createElement('div', { className: 'w-full flex gap-1 items-end justify-center h-48' },
                  React.createElement('div', {
                ...
                React.createElement('span', { className: 'text-xs text-slate-500 font-mono' }, day.date)
              )
            )
          )
        ),
        ...
        React.createElement('div', { className: 'cyber-card p-6' },
          ...
          React.createElement('div', { className: 'space-y-4' },
            CURRENCY_DATA.map(currency =>
              React.createElement('div', { key: currency.code, className: 'space-y-2' },
                React.createElement('div', { className: 'flex items-center justify-between' },
                  React.createElement('div', { className: 'flex items-center gap-2' },
                    React.createElement('span', {}, currency.flag),
                    React.createElement('span', { className: 'text-white font-medium' }, currency.code)
                  ),
                  React.createElement('span', { className: 'text-cyan-400 font-mono text-sm' }, `$${(currency.spend / 1000).toFixed(1)}K`)
                ),
                React.createElement('div', { className: 'data-bar' },
                  React.createElement('div', {
                    className: 'data-bar-fill',
                    style: { width: `${currency.percentage}%` }
                  })
                ),
                React.createElement('p', { className: 'text-xs text-slate-500 text-right' }, `${currency.percentage}% du total`)
              )
            )
          )
        )
        ...
        React.createElement('div', { className: 'overflow-x-auto' },
          React.createElement('table', { className: 'w-full' },
            React.createElement('thead', {},
              React.createElement('tr', { className: 'text-left text-xs text-slate-500 font-mono uppercase' },
            ...
            React.createElement('tbody', {},
              PLATFORM_BREAKDOWN.map(platform =>
                React.createElement('tr', { key: platform.name, className: 'border-t border-white/5 hover:bg-white/5 transition-colors' },
                  React.createElement('td', { className: 'py-4 pr-6' },
                    React.createElement('span', { className: 'text-white font-semibold' }, platform.name)
                  ),
                  React.createElement('td', { className: 'py-4 pr-6 font-mono text-slate-300' }, `$${(platform.spend / 1000).toFixed(1)}K`),
                  React.createElement('td', { className: 'py-4 pr-6 font-mono text-green-400' }, `$${(platform.revenue / 1000).toFixed(1)}K`),
                  React.createElement('td', { className: 'py-4 pr-6 font-mono text-cyan-400 font-bold' }, `${platform.roas}x`),
                  React.createElement('td', { className: 'py-4 pr-6 font-mono text-slate-300' }, `$${platform.cpa}`),
                  React.createElement('td', { className: 'py-4 pr-6 font-mono text-slate-300' }, platform.conversions.toLocaleString()),
                  React.createElement('td', { className: 'py-4' },
                    React.createElement('div', { className: 'flex items-center gap-2' },
                      React.createElement('div', { className: 'w-16 h-2 rounded-full bg-white/10 overflow-hidden' },
                        React.createElement('div', {
                          className: 'h-full bg-gradient-to-r from-cyan-500 to-purple-500',
                          style: { width: `${platform.share}%` }
                        })
                      ),
                      React.createElement('span', { className: 'text-xs text-slate-500 font-mono' }, `${platform.share}%`)
                    )
                  )
                )
              )
      ...
      React.createElement('div', { className: 'grid grid-cols-1 lg:grid-cols-2 gap-6' },
        ...
        React.createElement('div', { className: 'cyber-card cyber-card-orange p-6' },
          React.createElement('div', { className: 'flex items-center justify-between mb-6' },
            React.createElement('h2', { className: 'text-lg font-bold text-white flex items-center gap-2' },
          ...
          React.createElement('div', { className: 'space-y-3 max-h-80 overflow-y-auto pr-2' },
            ALERTS_HISTORY.map(alert =>
              React.createElement('div', {
                key: alert.id,
                ...
                React.createElement('div', { className: 'flex items-start justify-between gap-4' },
                  React.createElement('div', { className: 'flex-1' },
                    React.createElement('p', { className: 'text-sm text-white mb-1' }, alert.message),
                    React.createElement('div', { className: 'flex items-center gap-2 text-xs text-slate-500' },
                      React.createElement(Clock, { size: 12 }),
                      React.createElement('span', {}, `${alert.date} à ${alert.time}`)
                    )
                  ),
                  React.createElement('div', {
        ...
        React.createElement('div', { className: 'cyber-card cyber-card-purple p-6' },
          React.createElement('div', { className: 'flex items-center justify-between mb-6' },
            React.createElement('h2', { className: 'text-lg font-bold text-white flex items-center gap-2' },
            ...
            SEASONAL_EVENTS.map((event, idx) =>
              React.createElement('div', {
                ...
                React.createElement('div', { className: 'flex items-center justify-between mb-2' },
                  React.createElement('h3', { className: 'text-white font-semibold' }, event.name),
                ...
                React.createElement('div', { className: 'flex items-center justify-between text-sm' },
                  ...
                  React.createElement('span', { className: 'text-purple-400 font-mono' }, `J-${event.daysUntil}`)
                ),
                React.createElement('div', { className: 'mt-2 flex items-center gap-2' },
                  React.createElement(TrendingUp, { size: 14, className: 'text-green-400' }),
                  React.createElement('span', { className: 'text-xs text-slate-500' }, `Impact estimé: x${event.multiplier} sur conversions`)
                )
              )
            )
          ),
