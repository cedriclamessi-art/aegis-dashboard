    ...
    React.createElement('div', { className: 'max-w-6xl mx-auto' },
      ...
      React.createElement('div', { className: 'mb-12' },
        React.createElement('h1', { className: 'text-4xl font-bold mb-3 flex items-center gap-3' },
          React.createElement(TrendingUp, { size: 40, className: 'text-cyan-400' }),
          'Connectez vos Plateformes'
        ),
        React.createElement('p', { className: 'text-slate-300 text-lg' },
          'Autorisez AEGIS à accéder à vos comptes publicitaires pour gérer automatiquement vos campagnes'
        )
      ),
        ...
        platforms.map((platform) =>
          React.createElement('div',
            ...
            React.createElement('div', { className: 'flex items-center gap-4 mb-6' },
              React.createElement('div', { className: `w-12 h-12 ${platform.color} rounded-lg flex items-center justify-center` },
                React.createElement(platform.icon, { size: 24, className: 'text-white' })
              ),
              React.createElement('h2', { className: 'text-2xl font-bold' }, platform.name)
            ),
            
            React.createElement('p', { className: 'text-slate-300 mb-4' }, platform.description),
            
            React.createElement('div', { className: 'mb-6' },
              React.createElement('p', { className: 'text-sm text-slate-400 mb-3 font-semibold' }, 'Fonctionnalités:'),
              React.createElement('ul', { className: 'space-y-2' },
                platform.features.map((feature, idx) =>
                  React.createElement('li',
                    { key: idx, className: 'flex items-center gap-2 text-sm text-slate-300' },
                    React.createElement('span', { className: 'w-1.5 h-1.5 bg-cyan-400 rounded-full' }),
                    feature
                  )
                )
              )
            ...
            React.createElement('p', { className: 'text-xs text-slate-500 mt-4 text-center' },
              'Pas de données persistantes • Données simulées pour la démo'
            )
          )
        )
      )
