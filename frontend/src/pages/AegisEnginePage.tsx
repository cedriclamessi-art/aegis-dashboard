...
const C = {
    ...
    row: { display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 12 },
...
export default function AegisEnginePage() {
    const [opp, setOpp] = useState<OpportunityInput>({
          product_name: '', product_category: 'beauty',
          estimated_margin: 60, market_saturation: 40, competition_level: 50,
          trend_score: 70, avg_order_value: 45, capital_available: 1000,
          channel_ids: ['meta', 'tiktok'],
    });
  ...
  const doScore = async () => {
        setOppLoad(true);
        try { setOppRes(await scoreOpportunity(opp)); }
        catch (e: any) { setOppRes({ error: e.message }); }
        setOppLoad(false);
  };
  ...
  return (
        <div style={C.page}>
                ...
                <div style={C.sub}>Capital → profit net ajuste au risque. 4 lois. 0 emotion.</div>div>
                <div style={C.grid}>
                          <div style={C.card}>
                      ...
                      <div style={C.res}>
                        {slRes.triggered
                                          ? <div style={C.red}>STOP-LOSS DECLENCHE — {slRes.reason}</div>div>
                                      : <div style={C.grn}>OK — ROAS au-dessus du seuil</div>div>}
